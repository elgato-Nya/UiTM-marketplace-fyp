...
jest.mock("mongoose", () => ({
  ...
  Types: {
    ObjectId: {
      isValid: jest.fn((id) => {
        // Mock implementation: return true for valid format strings
...
describe("User Model Unit Test", () => {
  describe("Static Validation Unit Test", () => {
    ...
    it("should only allow valid usernames", () => {
      ...
      expect(isValidUsername("user12")).toBe(true); // exactly 6 chars
      expect(isValidUsername("user123456789012")).toBe(true); // exactly 16 chars
      ...
      expect(isValidUsername("ab")).toBe(false); // too short (2 chars)
  ...
  describe("Additional Validator Functions", () => {
    ...
    it("should validate bio content", () => {
      ...
      expect(isValidBio("A".repeat(250))).toBe(true); // exactly 250 chars

      // Invalid bios
      expect(isValidBio("A".repeat(251))).toBe(false); // too long
      expect(isValidBio(123)).toBe(false); // not a string
    });

    it("should validate campus enum values", () => {
      // Valid campus values - test both enum keys and display values
      expect(isValidCampus("SEGAMAT")).toBe(true);
      expect(isValidCampus("UiTM Kampus Segamat")).toBe(true);
      expect(isValidCampus("ARAU")).toBe(true);
    ...
    it("should validate faculty enum values", () => {
      // Valid faculty values - test both enum keys and display values
      expect(isValidFaculty("APPLIED_SCIENCE")).toBe(true);
      expect(isValidFaculty("Fakulti Sains Gunaan")).toBe(true);
      expect(isValidFaculty("COMPUTER_SCIENCE_MATH")).toBe(true);
    ...
    it("should return error messages object", () => {
      expect(typeof userErrorMessages).toBe("object");
      expect(userErrorMessages.email).toBeDefined();
      expect(userErrorMessages.password).toBeDefined();
      expect(userErrorMessages.username).toBeDefined();
      expect(userErrorMessages.phoneNumber).toBeDefined();
      expect(userErrorMessages.roles).toBeDefined();
      expect(userErrorMessages.campus).toBeDefined();
      expect(userErrorMessages.faculty).toBeDefined();
      expect(userErrorMessages.bio).toBeDefined();

      // Test that error userErrorMessages have the correct nested structure
      function validateErrorMessages(obj, path = "") {
        Object.entries(obj).forEach(([key, value]) => {
          const currentPath = path ? `${path}.${key}` : key;
          if (typeof value === "string") {
            expect(value.length).toBeGreaterThan(0);
          } else if (typeof value === "object" && value !== null) {
            validateErrorMessages(value, currentPath);
          } else {
            throw new Error(
              `Invalid error message structure at ${currentPath}: ${typeof value}`
            );
          }
        });
      }
      validateErrorMessages(userErrorMessages);
    });
  });
